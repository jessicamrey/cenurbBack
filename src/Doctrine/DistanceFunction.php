...
/**
 * "mydistance" "(" LatitudeFrom, LongitudetFrom, LatitudeTo, LongitudeTo ")"
 * 
...
class DistanceFunction extends FunctionNode {
    ...
    public function getSql(\Doctrine\ORM\Query\SqlWalker $sqlWalker) {
        ...
        $sql = '(' . $earthDiameterInKM . 
                ' * ASIN(SQRT(POWER(' .
                'SIN((' . $this->fromLat->dispatch($sqlWalker) . 
                ' - ABS(' . $this->toLat->dispatch($sqlWalker) . 
                ')) * PI() / 180 / 2), 2) + ' .
                'COS(' . $this->fromLat->dispatch($sqlWalker) . 
                ' * PI() / 180) * COS(ABS(' . $this->toLat->dispatch($sqlWalker) . ') * PI() / 180) * ' .
